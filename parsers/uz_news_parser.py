...
WIKI_CATEGORIES = [
    # Категории для разнообразия тем и стиля — можно расширять
    "O'zbekiston tarixi",
    "Fizika",
    "Iqtisodiyot",
    "Tibbiyot",
]
MAX_WIKI_ARTICLES_PER_CATEGORY = 40
...
def fetch_wiki_category_articles(category: str, limit: int) -> list[dict]:
    """
    Получает список статей категории и их полный текст через MediaWiki API.
    ...
    titles = [m["title"] for m in members if not m["title"].startswith("Kategoriya:")]
    if not titles:
        return []
