...
MAX_WIKI_ARTICLES_PER_CATEGORY = 120  # было 80 — с обходом на 2 уровня вглубь есть смысл поднять потолок
...
def fetch_category_member_titles(category: str, limit: int, max_subcat_depth: int = 2) -> list[str]:
    """
    ...
    Первый прогон с обходом на 1 уровень вглубь дал прирост (196 -> 249
    чанков), но всё ещё мало — 55 уникальных статей на 14 категорий вместо
    ожидаемых сотен. Причина: сами подкатегории первого уровня тоже часто
    почти пустые и держат содержательные статьи в СВОИХ подкатегориях
    (второй уровень вложенности). Обход до 2 уровней вглубь. Рекурсия
    останавливается сама, как только набрано `limit` статей — так что
    взрыва запросов при большой вложенности не будет.
    """
    titles: list[str] = []
